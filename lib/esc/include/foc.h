...
inline void park(const Vector2D &stat, float theta, Vector2D &res) {
...
inline void clarke(const Vector3D &natural, Vector2D &res) {
    res.x = natural.x;  // I_alpha
    res.y = (natural.x + 2.0f * natural.y) * 0.577350269f; // I_beta
}

// rotating frame to stationary frame
inline void park_inverse(const Vector2D &rot, float theta, Vector2D &res) {
...
inline void clarke_inverse(const Vector2D &stat, Vector3D &res) {
    ...
    float beta_calc  = stat.y * 0.866025403f;
